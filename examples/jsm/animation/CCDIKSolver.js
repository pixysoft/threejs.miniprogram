/**
 * backport from r185 (升级差距分析说明 3.0 §2.3)
 *
 * 与官方 r185 的 r110 适配差异:
 * 1. SphereGeometry(r185 为 BufferGeometry 版) → SphereBufferGeometry(r110 同语义类);
 * 2. Quaternion.invert() → inverse()(r110 方法名, 单位四元数下同语义);
 * 3. 相比 r110 旧版(MMD 附属 prototype 写法): 新增 update(globalBlendFactor) /
 *    逐 IK blendFactor(IK 结果与原动画按权重混合), 配合 additive 动画可做
 *    "上半身 IK + 下半身走路"。
 */
	...
	updateOne( ik, overrideBlend = 1.0 ) {
		...
		for ( let i = 0; i < iteration; i ++ ) {
			...
			for ( let j = 0, jl = links.length; j < jl; j ++ ) {
				...
				_invLinkQ.inverse(); // r110 适配: r185 为 invert()
...
class CCDIKHelper extends Object3D {
	...
	constructor( mesh, iks = [], sphereSize = 0.25 ) {
		 ...
		 * @type {SphereGeometry}
		 */
		this.sphereGeometry = new SphereBufferGeometry( sphereSize, 16, 8 ); // r110 适配
